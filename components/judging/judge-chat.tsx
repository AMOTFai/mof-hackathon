...
// Judges-only channel — same bubble UX as TeamChat, scoped by event_id
// instead of team_id (channel_type='judge', see migration 0005).
export function JudgeChat({
  ...
  useEffect(() => {
    ...
    const channel = getOrSubscribeChannel(supabase, `judge-chat-${eventId}`, (ch) =>
      ch.on(
        "postgres_changes",
        ...
        async (payload) => {
          ...
          if (row.channel_type !== "judge") return;
  ...
  }, [eventId, userId, userName, userAvatarUrl]);
  ...
  async function onSend(event: React.FormEvent) {
    ...
    const { data, error: sendError } = await supabase
      .from("messages")
      .insert({ event_id: eventId, channel_type: "judge", team_id: null, sender_id: userId, body: text })
  ...
  return (
    <div className="glass flex flex-col overflow-hidden p-0">
      <div ref={scrollRef} className="flex h-[28rem] flex-col gap-1 overflow-y-auto p-4" data-testid={`judge-chat-${eventId}`}>
        {messages.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center gap-1 text-center">
            <p className="text-sm text-muted-foreground">No messages yet.</p>
            <p className="text-xs text-muted-foreground">A private channel for judges on this event.</p>
          </div>
        ) : (
          <AnimatePresence initial={false}>
            {messages.map((message, i) => {
              ...
              return (
                <motion.div
                  key={message.id}
                  data-testid={`judge-chat-msg-${message.id}`}
                ...
                >
                  ...
                  <div className={cn("flex max-w-[75%] flex-col gap-0.5", isOwn ? "items-end" : "items-start")}>
                    {isFirstInGroup && !isOwn ? (
                      <p className="px-1 text-xs font-medium text-muted-foreground">{message.senderName || "Judge"}</p>
                    ) : null}
                    <div
                      className={cn(
      ...
      <form onSubmit={onSend} className="flex gap-2 border-t border-panel-border p-3">
        <Input
          ...
          placeholder="Message the judging panel"
          maxLength={4000}
          autoComplete="off"
          data-testid={`judge-chat-input-${eventId}`}
        />
        <Button type="submit" disabled={body.trim().length === 0} data-testid={`judge-chat-send-${eventId}`}>
          Send
        </Button>
      </form>
      {error ? (
